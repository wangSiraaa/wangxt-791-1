...
const startServer = () => {
  return new Promise((resolve, reject) => {
    ...
    serverProcess.stdout.on('data', (data) => {
      ...
      if (msg.includes('Server is ready') && !started) {
        started = true;
  ...
  try {
    ...
    const orderRes = await request({
      ...
      samples: [{
        ...
        sample_quantity: 500,
        sample_unit: 'ml',
        vial_barcode: 'TEST-VIAL-ACCEPT-' + Date.now(),
    ...
    if (orderRes.status === 201) {
      ...
      vialBarcode = orderRes.data.order_no ? '' : '';
      log(`委托单ID: ${orderId}`);
      log(`委托单号: ${orderRes.data.order_no}`);
    } else {
      testFail('创建委托单', `状态: ${orderRes.status}, 错误: ${orderRes.data?.error || '未知'}`);
    }
    console.log('');
    ...
    if (assignBeforeSample.status === 400) {
      if (assignBeforeSample.data?.rule_code === 'SAMPLE_NOT_RECEIVED') {
      ...
      testFail('规则校验失败', `未收样样品分派状态: ${assignBeforeSample.status}`);
    }
    console.log('');
    ...
    if (abnormalReceive.status === 200) {
      ...
      testFail('异常收样失败', `状态: ${abnormalReceive.status}, 错误: ${abnormalReceive.data?.error || '未知'}`);
    }
    console.log('');
    ...
    if (assignAbnormal.status === 400) {
      testPass('规则校验生效 - 异常样品不能分派');
      log(`返回: ${JSON.stringify(assignAbnormal.data).substring(0, 100)}`);
    } else {
      testFail('规则校验失败', `异常样品分派状态: ${assignAbnormal.status}`);
    }
    console.log('');
